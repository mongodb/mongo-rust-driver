...
fn convert_read_preference(test_read_pref: TestReadPreference) -> ReadPreference {
    let mode = match &test_read_pref.mode[..] {
        "Primary" => Mode::Primary,
        "Secondary" => Mode::Secondary(test_read_pref.tag_sets.unwrap_or_else(Vec::new)),
        "PrimaryPreferred" => {
            Mode::PrimaryPreferred(test_read_pref.tag_sets.unwrap_or_else(Vec::new))
        }
        "SecondaryPreferred" => {
            Mode::SecondaryPreferred(test_read_pref.tag_sets.unwrap_or_else(Vec::new))
        }
        "Nearest" => Mode::Nearest(test_read_pref.tag_sets.unwrap_or_else(Vec::new)),
