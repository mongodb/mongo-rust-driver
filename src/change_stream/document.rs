...
/// A `ChangeStreamEventDocument` represents a
...
pub struct ChangeStreamEventDocument {
    ...
    /// Displays the new name for the ns collection. Only displayed for `operationType` :
    /// `rename` and omitted for all other values of `operationType`.
