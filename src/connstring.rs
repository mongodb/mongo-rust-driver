...
use crate::{
    ...
    read_preference::{Mode, ReadPreference, TagSet},
};
...
pub struct ConnectionString {
    ...
    read_preference_mode: Option<Mode>,
...
impl ConnectionString {
    ...
    fn parse_options(&mut self, options: &str) -> Result<()> {
        ...
        let mode = match (
            self.read_preference_mode.take(),
            self.read_preference_tags.take(),
        ) {
            (None, None) => None,
            (Some(Mode::Primary), None) => Some(Mode::Primary),
            (Some(Mode::Primary), _) | (None, _) => bail!(ErrorKind::ArgumentError(
                "read preference tags can only be specified when a non-primary mode is specified"
                    .to_string()
            )),
            (Some(Mode::Secondary(_)), tags) => Some(Mode::Secondary(tags.unwrap_or_default())),
            (Some(Mode::PrimaryPreferred(_)), tags) => {
                Some(Mode::PrimaryPreferred(tags.unwrap_or_default()))
            }
            (Some(Mode::SecondaryPreferred(_)), tags) => {
                Some(Mode::SecondaryPreferred(tags.unwrap_or_default()))
            }
            (Some(Mode::Nearest(_)), tags) => Some(Mode::Nearest(tags.unwrap_or_default())),
        };
    ...
    fn parse_option_pair(&mut self, key: &str, value: &str) -> Result<()> {
        ...
        match key {
            ...
            "readpreference" => {
                self.read_preference_mode = Some(match &value.to_lowercase()[..] {
                    "primary" => Mode::Primary,
                    "secondary" => Mode::Secondary(Vec::new()),
                    "primarypreferred" => Mode::PrimaryPreferred(Vec::new()),
                    "secondarypreferred" => Mode::SecondaryPreferred(Vec::new()),
                    "nearest" => Mode::Nearest(Vec::new()),
    ...
    fn with_mixed_options() {
        ...
        assert_eq!(
            ...
            ConnectionString {
                ...
                read_preference: Some(ReadPreference {
                    mode: Mode::SecondaryPreferred(vec![
                        tag_set! {
                            "dc" => "ny",
                            "rack" => "1"
                        },
