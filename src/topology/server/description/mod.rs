...
impl ServerDescription {
    ...
    pub fn matches(&self, topology_type: TopologyType, read_pref: Option<&ReadPreference>) -> bool {
        match read_pref {
            Some(pref) => self.matches_mode(topology_type, &pref.mode),
            None => true,
        }
    }

    fn matches_mode(&self, topology_type: TopologyType, mode: &Mode) -> bool {
        match mode {
            Mode::Primary => self.server_type == ServerType::RSPrimary,
            Mode::Secondary(tag_sets) => {
                self.server_type == ServerType::RSSecondary && self.matches_tag_sets(&tag_sets)
            }
            Mode::PrimaryPreferred(tag_sets) => {
                self.server_type == ServerType::RSPrimary
                    || (topology_type == TopologyType::ReplicaSetNoPrimary
                        ...
                        && self.matches_tag_sets(&tag_sets)
            }
            ...
            Mode::Nearest(tag_sets) => self.matches_tag_sets(&tag_sets),
        }
    }

    fn matches_tag_sets(&self, tag_sets: &[TagSet]) -> bool {
