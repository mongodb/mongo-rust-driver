...
use crate::{
    ...
    options::{CreateCollectionOptions, EncryptOptions, FindOptions, IndexOptions, WriteConcern},
    runtime,
    test::{
        get_client_options,
        log_uncaptured,
...
// Prose test 25. $lookup
mod lookup {
    use super::*;

    async fn set_up() -> Option<Client> {
        if mongocrypt_version_lt("1.13.0")
            || server_version_lt(7, 0).await
            || topology_is_standalone().await
        {
            log_uncaptured(
                "skipping $lookup test: requires mongocrypt 1.13+, server 7.0+, non-standalone \
                 topology",
            );
            return None;
        }
        ...
        let keyvault = encrypted_client.database("db").collection("keyvault");
        keyvault.drop().await.unwrap();
        let key_doc: Document = load_testdata("data/lookup/key-doc.json").unwrap();
        keyvault
            .insert_one(key_doc)
            .write_concern(WriteConcern::majority())
            .await
        ...
        let drop_and_create = async |coll: &str, options: Option<CreateCollectionOptions>| {
            let db = encrypted_client.database("db");
            db.collection::<Document>(coll).drop().await.unwrap();
        ...
        let schema_csfle: Document = load_testdata("data/lookup/schema-csfle.json").unwrap();
        drop_and_create(
            "csfle",
            Some(
                CreateCollectionOptions::builder()
                    .validator(doc! { "$jsonSchema": schema_csfle })
                    .build(),
            ),
        )
        .await;

        let schema_csfle2: Document = load_testdata("data/lookup/schema-csfle2.json").unwrap();
        drop_and_create(
            "csfle2",
            Some(
                CreateCollectionOptions::builder()
                    .validator(doc! { "$jsonSchema": schema_csfle2 })
                    .build(),
            ),
        )
        .await;

        let schema_qe: Document = load_testdata("data/lookup/schema-qe.json").unwrap();
        drop_and_create(
            "qe",
            Some(
                CreateCollectionOptions::builder()
                    .encrypted_fields(schema_qe)
                    .build(),
            ),
        )
        .await;

        let schema_qe2: Document = load_testdata("data/lookup/schema-qe2.json").unwrap();
        drop_and_create(
            "qe2",
            Some(
                CreateCollectionOptions::builder()
                    .encrypted_fields(schema_qe2)
                    .build(),
            ),
        )
        .await;

        drop_and_create("no_schema", None).await;
        drop_and_create("no_schema2", None).await;

        let schema_non_csfle: Document =
            load_testdata("data/lookup/schema-non-csfle.json").unwrap();
        drop_and_create(
            "non_csfle_schema",
            Some(
                CreateCollectionOptions::builder()
                    .validator(doc! { "$jsonSchema": schema_non_csfle })
                    .build(),
            ),
        )
        .await;

        let unencrypted_client = Client::for_test().await;
        let insert = async |name: &str| -> Bson {
            let coll = encrypted_client.database("db").collection(name);
            let doc = doc! { name: name };
            coll.insert_one(doc).await.unwrap().inserted_id
        };
        let insert_and_find = async |name: &str| {
            let inserted_id = insert(name).await;
            let coll = unencrypted_client
                .database("db")
            ...
            let inserted = coll
                ...
                .unwrap()
                .unwrap();
            let value = inserted.get(name).unwrap();
            assert!(matches!(value, Bson::Binary(_)));
        };

        insert_and_find("csfle").await;
        insert_and_find("csfle2").await;
        insert_and_find("qe").await;
        insert_and_find("qe2").await;
        insert("no_schema").await;
        insert("no_schema2").await;
        insert("non_csfle_schema").await;

        Some(
            Client::encrypted_builder(
                get_client_options().await.clone(),
                Namespace::new("db", "keyvault"),
                [LOCAL_KMS.clone()],
            )
    ...
    async fn test_successful_lookup(coll: &str, pipeline: Vec<Document>, expected: Document) {
        if server_version_lt(8, 1).await {
            log_uncaptured("skipping $lookup test: requires server version 8.1+");
            return;
        }
        let Some(client) = set_up().await else {
            return;
        };

        let coll = client.database("db").collection::<Document>(coll);
        let mut cursor = coll.aggregate(pipeline).await.unwrap();
    ...
    async fn case_1() {
        ...
        let expected = doc! { "csfle": "csfle", "matched": [ { "no_schema": "no_schema" } ] };
        test_successful_lookup(coll, pipeline, expected).await;
    }
    ...
    async fn case_2() {
        ...
        let pipeline = vec![
            ...
            doc! { "$project": { "_id": 0, "__safeContent__": 0 } },
        ];
        let expected = doc! { "qe": "qe", "matched": [ { "no_schema": "no_schema" } ] };
        test_successful_lookup(coll, pipeline, expected).await;
    }
    ...
    async fn case_3() {
        ...
        let expected = doc! { "no_schema": "no_schema", "matched": [ { "csfle": "csfle" } ] };
        test_successful_lookup(coll, pipeline, expected).await;
    }
    ...
    async fn case_4() {
        ...
        let expected = doc! { "no_schema": "no_schema", "matched": [ { "qe": "qe" } ] };
        test_successful_lookup(coll, pipeline, expected).await;
    }
    ...
    async fn case_5() {
        ...
        let expected = doc! { "csfle": "csfle", "matched": [{ "csfle2": "csfle2" }] };
        test_successful_lookup(coll, pipeline, expected).await;
    }
    ...
    async fn case_6() {
        ...
        let expected = doc! { "qe": "qe", "matched": [ { "qe2": "qe2" } ] };
        test_successful_lookup(coll, pipeline, expected).await;
    }
    ...
    async fn case_7() {
        ...
        let expected =
            doc! { "no_schema": "no_schema", "matched": [ { "no_schema2": "no_schema2" } ] };
        test_successful_lookup(coll, pipeline, expected).await;
    }
    ...
    async fn case_8() {
        ...
        let coll = client.database("db").collection::<Document>("csfle");
        let pipeline = vec![
            doc! { "$match": { "csfle": "qe" } },
            doc! {
                "$lookup": {
        ...
        let error = coll.aggregate(pipeline).await.unwrap_err();

        let message = error.message().unwrap();
        if server_version_lt(8, 2).await || mongocrypt_version_lt("1.17.0") {
    ...
    #[tokio::test]
    async fn case_9() {
        if server_version_gte(8, 1).await {
            log_uncaptured("skipping $lookup test: requires server version <8.1");
            return;
        }
        let Some(client) = set_up().await else {
        ...
        let error = coll.aggregate(pipeline).await.unwrap_err();
        assert!(error.message().unwrap().contains("Upgrade"));
    }
    ...
    async fn case_10() {
        if server_version_lt(8, 2).await || mongocrypt_version_lt("1.17.0") {
            log_uncaptured(
                "skipping $lookup test: requires server version 8.2+, mongocrypt 1.17.0+",
            );
            return;
        }
        ...
        let expected =
            doc! { "qe": "qe", "matched": [ { "non_csfle_schema": "non_csfle_schema" } ] };
        test_successful_lookup(coll, pipeline, expected).await;
    }
}
...
async fn text_indexes_explicit_encryption() {
    ...
    {
        log_uncaptured(
            "skipping text_indexes_explicit_encryption: requires non-standalone topology, 8.2+, \
    ...
    // Client setup
    type Setup = TextIndexesExplicitEncryptionSetup;

    async fn skip_on_9(label: &str) -> bool {
        let out = server_version_lt(9, 0).await;
        if !out {
            log_uncaptured(format!(
                "skipping text explicit encryption [{}]: requires server < 9.0",
                label
            ));
        }
        out
    ...
    if skip_on_9("can find a document by prefix").await {
        let Setup {
        ...
        let actual = prefix_suffix_coll(&explicit_encrypted_client)
    ...
    if skip_on_9("can find a document by suffix").await {
        let Setup {
            explicit_encrypted_client,
            client_encryption,
            key1_id,
    ...
    if skip_on_9("assert no document found by prefix").await {
        let Setup {
            explicit_encrypted_client,
            client_encryption,
            key1_id,
    ...
    if skip_on_9("assert no document found by suffix").await {
        let Setup {
            explicit_encrypted_client,
            client_encryption,
            key1_id,
    ...
    // Case 5: can find a document by substring
    {
        ...
        let actual = substring_coll(&explicit_encrypted_client)
    ...
    if skip_on_9("assert contentionFactor is required").await {
        let Setup {
            client_encryption,
            key1_id,
            ..
    ...
    if skip_on_9("insensitive prefix/suffix").await {
        for (text, prefix, suffix) in [
            // Case 8: can find an auto-encrypted case indexed document by prefix and suffix
            ("BingQiLin", "bing", "lin"),
            // Case 9: can find an auto-encrypted diacritic-insensitively indexed document by
            // prefix and suffix
            ("cafébarbäz", "cafe", "baz"),
        ] {
            let Setup {
                explicit_encrypted_client,
                auto_encrypted_client,
                client_encryption,
                key1_id,
                ..
            } = Setup::new().await;

            let id = auto_encrypted_client
                .database("db")
                .collection::<Document>("prefix-suffix-ci-di")
                .insert_one(doc! { "encryptedText": text })
                .write_concern(WriteConcern::majority())
                .await
                .unwrap()
                .inserted_id;
            ...
            let coll = explicit_encrypted_client
                .database("db")
                .collection::<Document>("prefix-suffix-ci-di");

            let found = coll
                .find_one(prefix_filter(encrypted_pre))
                .projection(doc! { "__safeContent__": 0 })
                .await
                .unwrap();
            assert_eq!(
                Some(doc! { "_id": id.clone(), "encryptedText": text }),
                found
            );
            ...
            let found = coll
                .find_one(suffix_filter(encrypted_suf))
                .projection(doc! { "__safeContent__": 0 })
                .await
                .unwrap();
            assert_eq!(Some(doc! { "_id": id, "encryptedText": text }), found);
        }
    }
    ...
    for (text, substring) in [
        // Case 10: can find an auto-encrypted case-insensitively indexed document by substring
        ("FooBarBaz", "bar"),
        // Case 11: can find an auto-encrypted diacritic-insensitively indexed document by
        // substring
        ("foocafébaz", "cafe"),
    ] {
        let Setup {
        ...
        let id = auto_encrypted_client
            .database("db")
            .collection::<Document>("substring-ci-di")
            .insert_one(doc! { "encryptedText": text })
            .write_concern(WriteConcern::majority())
            .await
            .unwrap()
        ...
        let found = coll
            .find_one(substring_filter(encrypted_sub))
            .projection(doc! { "__safeContent__": 0 })
            .await
            .unwrap();
        assert_eq!(
...
#[cfg(feature = "text-indexes-unstable")]
struct TextIndexesExplicitEncryptionSetup {
    explicit_encrypted_client: Client,
    auto_encrypted_client: Client,
    client_encryption: ClientEncryption,
    key1_id: Binary,
}

#[cfg(feature = "text-indexes-unstable")]
impl TextIndexesExplicitEncryptionSetup {
    async fn new() -> Self {
        ...
        for (path, coll, skip_on_9) in [
            ...
            db.collection::<Document>(coll)
                .drop()
                .encrypted_fields(enc_fields.clone())
                .await
                .unwrap();
            db.create_collection(coll)
        ...
        let explicit_encrypted_client = Client::encrypted_builder(
            get_client_options().await.clone(),
            datakeys.namespace(),
            [LOCAL_KMS.clone()],
        )
        ...
        let auto_encrypted_client = Client::encrypted_builder(
            get_client_options().await.clone(),
            datakeys.namespace(),
            [LOCAL_KMS.clone()],
        )
