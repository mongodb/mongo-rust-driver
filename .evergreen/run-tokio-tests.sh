...
RUST_BACKTRACE=1 cargo test sync --features sync $OPTIONS | tee sync-tests.json
cat sync-tests.json | cargo2junit > sync-tests.xml
RUST_BACKTRACE=1 cargo test --doc sync --features sync $OPTIONS | tee sync-doc-tests.json
