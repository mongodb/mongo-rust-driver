...
```bash
aws-lambda-test$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
aws-lambda-test$ sam local start-api
aws-lambda-test$ curl http://localhost:3000/
```
...
```bash
aws-lambda-test$ sam logs -n HelloWorldFunction --stack-name aws-lambda-test --tail
```
