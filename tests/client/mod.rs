...
// /// description and check if it contains "Authentication". Once R2D2 is removed, this function
// can /// go away and we can just check for our own auth error types.
...
// /// Attempts to authenticate using the given username/password, optionally specifying a
// mechanism, /// via the `ClientOptions` api.
// ///
...
// /// Tries to authenticate with the given credentials using the given mechanisms, both by
// explicitly /// specifying each mechanism and by relying on mechanism negotiation.
// ///
// /// If only one mechanism is supplied, this will also test that using the other SCRAM mechanism
// will /// fail.
