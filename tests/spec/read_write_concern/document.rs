...
use mongodb::concern::{Acknowledgment, WriteConcern};
...
fn run_document_test(test_file: TestFile) {
    for test_case in test_file.tests {
        if let Some(specified_write_concern) = test_case.write_concern {
            let actual_write_concern = match write_concern_from_document(specified_write_concern)
                .map(WriteConcern::into_document)
            {
                Some(Ok(write_concern)) => {
