...
fn run_find_one_and_delete_test(test_file: TestFile) {
    ...
    for mut test_case in test_file.tests {
        ...
        let coll = crate::init_db_and_coll("findOneAndDelete", &test_case.description);
        ...
        if let Some(ref c) = outcome.collection {
            if let Some(ref name) = c.name {
                crate::get_coll("findOneAndDelete", name)
                    .drop()
                    .expect(&test_case.description);
            }
        }
        ...
        if let Some(c) = outcome.collection {
            let outcome_coll = match c.name {
                Some(ref name) => crate::get_coll("findOneAndDelete", name),
                None => coll,
            };
...
#[test]
fn run() {
    crate::spec::test(&["crud", "write"], run_find_one_and_delete_test);
}
