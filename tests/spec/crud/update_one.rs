...
fn run_update_one_test(test_file: TestFile) {
    ...
    for mut test_case in test_file.tests {
        ...
        let coll = crate::init_db_and_coll("updateOne", &test_case.description);
        ...
        let result = coll
            .update_one(arguments.filter, arguments.update, Some(options))
            .expect(&test_case.description);
        assert_eq!(
        ...
        if let Some(c) = outcome.collection {
            let outcome_coll = match c.name {
                Some(ref name) => crate::get_coll("updateOne", name),
                None => coll,
            };
